...
Camera::Camera(int w, int h)
{
	...
	_boundaryLeft = 0;
	_boundaryRight = SCREEN_WIDTH;
	_boundaryTop = 0;
	_boundaryBottom = SCREEN_HEIGHT;;
	_xCamBackup = _yCamBackup = 0;
	SetBoundaryBackup(_boundaryLeft, _boundaryRight, _boundaryTop,_boundaryBottom);
	vx = 0;
}
...
D3DXVECTOR3 Camera::GetPostion()
{ 
	D3DXVECTOR3 p(trunc(GetXCam()), trunc(GetYCam()), 0);
	return p;
}
...
void Camera::SetBoundaryBackup(float l, float r, float t, float b)
{
	_boundaryLeftBackup = l;
	_boundaryRightBackup = r;
	_boundaryTopBackup = t;
	_boundaryBottomBackup = b;
}
...
void Camera::RestoreBoundary()
{
	...
	_boundaryTop = _boundaryTopBackup;
	_boundaryBottom = _boundaryBottomBackup;
}
