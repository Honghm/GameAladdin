...
CSprite::CSprite(CTexture* Texture, DWORD TimeAnimation)
{
	...
	widthframe = texture->frameWidth;
	heightframe = texture->frameHeight;
...
void CSprite::DrawFrame(int idFrame, float X, float Y, int alpha, int R, int G, int B)
{
	...
	//DrawScale(idFrame, X, Y, alpha, R, G, B);
...
int CSprite::GetWidthFrame()
{
	return widthframe;
}

int CSprite::GetHeightFrame()
{
	return heightframe;
}

void CSprite::SetTimeAnimation(DWORD time)
{
	this->timeAnimation = time;
}
