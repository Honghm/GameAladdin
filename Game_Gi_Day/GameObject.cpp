...
void GameObject::SubHealth(int h)
{
	Health -= h;
	if (Health < 0)
		Health = 0;
...
void GameObject::SetType(eType t)
{
	type = t;
}
...
void GameObject::FilterCollision(vector<LPCOLLISIONEVENT>& coEvents, vector<LPCOLLISIONEVENT>& coEventsResult, float & min_tx, float & min_ty, float & nx, float & ny) //kiểm tra hướng va chạm với các obj
{
	min_tx = 1.0f;
	min_ty = 1.0f;
